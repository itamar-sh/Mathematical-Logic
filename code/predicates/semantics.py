...
class Model(Generic[T]):
    ...
    def __init__(self, universe: AbstractSet[T],
        ...
        for function in function_interpretations:
            ...
            assert arity > 0
            whatthe = len(universe)**arity  # to delete
    ...
    def evaluate_term(self, term: Term,
        ...
        if is_function(term.root):
            cur_func = self.function_interpretations[term.root]
            list_args = list()
            for arg in term.arguments:
                list_args.append(self.evaluate_term(arg, assignment))
            return cur_func[tuple(list_args)]
    ...
    def evaluate_formula(self, formula: Formula,
        ...
        if is_relation(formula.root):
            cur_func = self.relation_interpretations[formula.root]
            list_args = list()
            for arg in formula.arguments:
                list_args.append(self.evaluate_term(arg, assignment))
            return tuple(list_args) in cur_func
        ...
        if is_quantifier(formula.root):
            if formula.root == "A":
                for element in self.universe:
                    new_dict = dict()
                    new_dict.update(assignment)
                    new_dict[formula.variable] = element
                    if not self.evaluate_formula(formula.statement, new_dict):
                        return False
                return True
            if formula.root == "E":
                for element in self.universe:
                    ...
                    new_dict.update(assignment)
                    new_dict[formula.variable] = element
                    if self.evaluate_formula(formula.statement, new_dict):
                        return True
                return False
    ...
    def is_model_of(self, formulas: AbstractSet[Formula]) -> bool:
        ...
        for formula in formulas:
            ...
            for var in free_vars:
                updated = Formula('A', var, updated)
            if not self.evaluate_formula(updated, dict()):
                return False
        return True
